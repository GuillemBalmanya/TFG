...
<xsl:text>awardUuid;awardText_ca_ES;awardText_en_GB;awardText_es_ES;colPureId;colTypePureId;colType_term_text_ca_ES;colType_term_text_en_GB;colType_term_text_es_ES;leadCollaborator;orgUuid;org_name_ca_ES;org_name_en_GB;org_name_es_ES;orgTypePureId;org_type_ca_ES;org_type_en_GB;org_type_es_ES</xsl:text>
		...
		<xsl:for-each select="$json-xml/fn:map/fn:array[@key='items']/fn:map">
			...
			<xsl:for-each select="fn:array[@key='collaborators']/fn:map">
				...
				<xsl:choose>
					<xsl:when test="organizationalUnit">
						<xsl:value-of select="fn:map[@key='organizationalUnit']/fn:string[@key='uuid']"/>
						<xsl:value-of select="$separator" />
						<xsl:value-of select="uab:clean_ca_en_es(fn:map[@key='organizationalUnit']/fn:string[@key='name'])"/>
						<xsl:value-of select="$separator" />
						<xsl:value-of select="fn:map[@key='organizationalUnit']/fn:map[@key='type']/fn:number[@key='pureId']"/>
						<xsl:value-of select="$separator" />
						<xsl:value-of select="uab:clean_ca_en_es(fn:map[@key='organizationalUnit']/fn:map[@key='type']/fn:map[@key='term'])"/>
						<xsl:value-of select="$separator" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="fn:map[@key='externalOrganization']/fn:string[@key='uuid']"/>
						<xsl:value-of select="$separator" />
						<xsl:value-of select="uab:clean_ca_en_es(fn:map[@key='externalOrganization']/fn:string[@key='name'])"/>
						<xsl:value-of select="$separator" />
