...
<xsl:text>uuid;text_ca_ES;text_en_GB;text_es_ES;typePureId;typeUri;type_term_text_ca_ES;type_term_text_en_GB;type_term_text_es_ES;managingOrganisationalUnit_Uuid;managingOrganisationalUnit_ExternaId;MOU_name_text_ca_ES;MOU_name_text_en_GB;MOU_name_text_es_ES;SN_collaborative;totalAwardedAmount;totalSpendAmount;actualPeriodStartDate;actualPeriodEndtDate;expectedPeriodStartDate;expectedPeriodEndtDate;awardDate;estatId;internallyApprovedDate;relinquished;relinquishmentDate;relinquishmentReason;declined;declinationDate;declinedReason;sn_aCarrecId;sn_aCarrec_term_text_ca_ES;sn_aCarrec_term_text_en_GB;sn_aCarrec_term_text_es_ES;workflow;pureId;natureTypePureId;natureTypeUri;natureType_term_text_ca_ES;natureType_term_text_en_GB;natureType_term_text_es_ES;recompte;justificacioEconomica</xsl:text>
		...
		<xsl:for-each select="$json-xml/fn:map/fn:array[@key='items']/fn:map">
			...
			<xsl:value-of
				select="fn:map[@key='managingOrganization']/fn:string[@key='uuid']" />
			<xsl:value-of select="$separator" />
			<xsl:value-of
				select="fn:map[@key='managingOrganization']/fn:string[@key='externalId']" />
			<xsl:value-of select="$separator" />
			<xsl:value-of
				select="uab:clean_ca_en_es(fn:map[@key='managingOrganization']/fn:map[@key='name'])" />		
			<xsl:value-of
				select="$separator" />
			...
			<!--<xsl:value-of
			select="fn:array[@key='keywordGroups']/fn:map
			[@logicalName='/uab/awards/estat_ajut']/fn:array[@key='keywordContainers']/fn:map/fn:map[@key='structuredKeyword']/fn:number[@key='pureId']"/>
			<xsl:value-of select="$separator" />
			<xsl:value-of select="uab:clean_ca_en_es(fn:array[@key='keywordGroups']/fn:map[fn:string[@key='logicalName']='/uab/awards/estat_ajut']/fn:array[@key='keywordContainers']/fn:map/fn:map[@key='structuredKeyword']/fn:map[@key='term'])"/>
			<xsl:value-of select="$separator" /> -->
			...
			<xsl:value-of
				select="uab:clean(fn:array[@key='keywordGroups']/fn:map[fn:string[@key='logicalName']='/uab/awards/a_carrec']/fn:array[@key='keywordContainers']/fn:map/fn:number[@key='pureId'])" />
			<xsl:value-of
				select="$separator" />
			<xsl:value-of
				select="uab:clean_ca_en_es(fn:array[@key='keywordGroups']/fn:map[fn:string[@key='logicalName']='/uab/awards/a_carrec']/fn:array[@key='keywordContainers']/fn:map/fn:map[@key='term'])" />
			...
			<xsl:variable name="nature" select="fn:array[@key='natureTypes']/fn:map" />
			
			
			<xsl:value-of select="$separator" />
			<xsl:value-of select="$nature/fn:number[@key='pureId']" />
			<xsl:value-of select="$separator" />
			<xsl:value-of select="$nature/fn:string[@key='uri']" />
			<xsl:value-of select="$separator" />
			<xsl:value-of select="uab:clean_ca_en_es($nature/fn:map[@key='term'])" />
			<xsl:value-of select="$separator" />
			<!--recvompte
			...
			<xsl:choose>
			<xsl:when test="fn:array[@key='keywordGroups']/fn:map[fn:string[@key='logicalName']='/uab/awards/justificacio_economica']">
			<xsl:choose>
			<xsl:when test="fn:array[@key='keywordGroups']/fn:map[fn:string[@key='logicalName']='/uab/awards/justificacio_economica']/fn:array[@key='keywordContainers']/fn:map/fn:map[@key='structuredKeyword'][fn:string[@key='uri']='/uab/awards/justificacio_economica/si']">
			<xsl:text>S</xsl:text>
			</xsl:when>
			<xsl:when test="fn:array[@key='keywordGroups']/fn:map[fn:string[@key='logicalName']='/uab/awards/justificacio_economica']/fn:array[@key='keywordContainers']/fn:map/fn:map[@key='structuredKeyword'][fn:string[@key='uri']='/uab/awards/justificacio_economica/no']">
			<xsl:text>N</xsl:text>
			</xsl:when>
			<xsl:otherwise>
			<xsl:text>N</xsl:text>
			...
			<xsl:when test="fn:array[@key='keywordGroups']/fn:map[fn:string[@key='logicalName']='/uab/awards/classificacio_ajut']/fn:array[@key='keywordContainers']/fn:map/fn:map[@key='structuredKeyword']/fn:number[@key='pureId'] = '31912012'">
			<xsl:text>S</xsl:text>
			</xsl:when>
			<xsl:otherwise>
			<xsl:text>N</xsl:text>
