...
<xsl:text>actUuid;PersonUuid;personRoleUuid;personRoleUri;personRoleText_ca;personRoleText_en;personRoleText_es</xsl:text>
		...
		<xsl:for-each select="$json-xml/fn:map/fn:array[@key='items']/fn:map">
			...
			<xsl:variable name="actUuid" select="fn:string[@key='uuid']" />

			<!-- Recorrem les persones de les activitats -->
			<xsl:for-each select="fn:array[@key='persons']/fn:map">

				<!-- Identificador de l'activitat que hem guardat abans-->

				<xsl:value-of select="$actUuid"/>
