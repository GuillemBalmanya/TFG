<xsl:stylesheet version="3.0"
	...
	<xsl:template match="/">
		...
		<xsl:text>prizeUuid;externalId;typePureId;typeUri;typeText_ca;typeText_en;typeText_es;titleText_ca;titleText_en;titleText_es;descriptionText_ca;descriptionText_en;descriptionText_es;awardDateYear;awardDateMonth;awardDateDay;degreeOfRecognitionPureId;degreeOfRecognitionUri;degreeOfRecognitionText_ca;degreeOfRecognitionText_en;degreeOfRecognitionText_es;workflowStep;createdBy;createdDate</xsl:text>
		...
		<xsl:for-each select="$json-xml/fn:map/fn:array[@key='items']/fn:map">
			...
			<xsl:value-of select="$separator" />	
			<!-- data del premi-->			
			<xsl:value-of select="fn:map[@key='awardedDate']/fn:number[@key='year']"/>
			<xsl:value-of select="$separator" />
			<xsl:value-of select="fn:map[@key='awardedDate']/fn:number[@key='month']"/>
			<xsl:value-of select="$separator" />
			<xsl:value-of select="fn:map[@key='awardedDate']/fn:number[@key='day']"/>
			<xsl:value-of select="$separator" />		
			<!-- ambit-->	
