...
<xsl:stylesheet version="3.0"
	...
	<xsl:template match="/" name="xsl:initial-template">
		...
		<xsl:text>ctbaUuid;ctbaTypePureId;ctbaTypeUri;ctbaType_ca_ES;ctbaType_en_GB;ctbaType_es_ES;ctbaTitle;ctbaExtenalId;ctbaHostPublicationTitle;ctbaEdition;ctbaISBN;ctbaCategoyrPureId;ctbaCategoryUri;ctbaCategory_ca_ES;ctbaCategory_en_GB;ctbaCategory_es_ES;ctbaLanguage;ctbaPlaceOfPublication;ctbaPublisher;ctbaPublicationSerie;ctbaVolume;ctbaWorkflowStep;ctbaCreatedDate;ctbaCreatedBy</xsl:text>
		<xsl:value-of select="$newline" />
		<xsl:for-each select="$json-xml/fn:map/fn:array[@key='items']/fn:map[fn:string[@key='typeDiscriminator']='ContributionToBookAnthology']">
			...
			<xsl:value-of select="uab:clean(fn:array[@key='isbns']/fn:map[1]/fn:string[@key='value'])"/><xsl:value-of select="$separator" />
