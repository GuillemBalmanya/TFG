	...
	<xsl:template match="/" name="xsl:initial-template">
		...
		<xsl:for-each select="$json-xml/fn:map/fn:array[@key='items']/fn:map">
			...
			<xsl:for-each select="fn:array[@key='contributors']/fn:map">
				...
				</xsl:choose><xsl:value-of select="$separator" />
				<xsl:value-of select="fn:map[@key='role']/fn:number[@key='pureId']"/><xsl:value-of select="$separator" />
				<xsl:value-of select="fn:map[@key='role']/fn:string[@key='uri']"/><xsl:value-of select="$separator" />
				<xsl:value-of select="uab:clean_ca_en_es(fn:map[@key='role']/fn:map[@key='term'])"/><xsl:value-of select="$newline" />
			</xsl:for-each>
		</xsl:for-each>
	</xsl:template> 
</xsl:stylesheet>