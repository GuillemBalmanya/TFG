<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:uab="http://www.uab.cat">
	...
	<xsl:template match="/">    
		...
		<xsl:text>uuid;externalId;text_ca_ES;text_en_GB;text_es_ES;programaId;programa_term_text_ca_ES;programa_term_text_en_GB;programa_term_text_es_ES</xsl:text>
		...
		<xsl:for-each select="result/items/fundingOpportunity">
			...
			<xsl:for-each select="keywordGroups/keywordGroup">
				<xsl:if test="@logicalName='/uab/fundingopportunities/programes'">
					...
					<xsl:value-of select="keywordContainers/keywordContainer/structuredKeyword/@pureId"/>
					<xsl:value-of select="$separator" />
					<xsl:value-of select="uab:clean_ca_en_es(keywordContainers/keywordContainer/structuredKeyword/term/text)"/>
