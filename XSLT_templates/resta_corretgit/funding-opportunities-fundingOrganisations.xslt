<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:uab="http://www.uab.cat">
	...
	<xsl:template match="/">    
		...
		<xsl:text>uuid;externalId;text_ca_ES;text_en_GB;text_es_ES;fundOrgUuid;fundOrgExternalId;fundOrgName;typePureId;TypeUri;Type_term_text_ca_ES;Type_term_text_en_GB;Type_term_text_es_ES</xsl:text>
		...
		<xsl:for-each select="result/items/fundingOpportunity">
			...
			<!-- Recorrem els organismes -->	
			<xsl:for-each select="fundingOrganisations/fundingOrganisation">
				...
				<!--organismes que financen la convoatoria-->
				<xsl:value-of select="@uuid"/>
				<xsl:value-of select="$separator" />
				<xsl:value-of select="uab:clean(@ externalId)"/>
				<xsl:value-of select="$separator" />
				<xsl:value-of select="uab:clean(name/text)"/>
				<xsl:value-of select="$separator" />				
