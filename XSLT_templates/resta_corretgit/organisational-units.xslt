...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:uab="http://www.uab.cat">
	...
	<xsl:template match="/">
		...
		<xsl:for-each select="result/items/organisationalUnit">
			...
			<xsl:value-of select="$separator" />
			<xsl:value-of
				select="uab:clean_ca_en_es(name/text)" />
			<xsl:value-of select="$separator" />
			<xsl:value-of
				select="type/@pureId" />
			<xsl:value-of select="$separator" />
			<xsl:value-of
				select="uab:clean_ca(type/term/text)" />
			<xsl:value-of select="$separator" />
			<!--
...
-->
			<xsl:value-of
				select="ids/id/type[@pureId=176]/../value" />
			<xsl:value-of select="$separator" />
			<xsl:for-each select="ids/id">
			...
			<!--ambit-->
			<xsl:value-of
				select="keywordGroups/keywordGroup [@logicalName='/uab/organisations/ambit_coneix']/keywordContainers/keywordContainer/structuredKeyword/@pureId" />
			<xsl:value-of
				select="$separator" />
			<xsl:value-of
				select="uab:clean_ca_en_es(keywordGroups/keywordGroup [@logicalName='/uab/organisations/ambit_coneix']/keywordContainers/keywordContainer/structuredKeyword/term/text)" />
			<xsl:value-of select="$separator" />
			<xsl:value-of
				select="@pureId" />
