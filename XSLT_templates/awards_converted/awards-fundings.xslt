...
<xsl:text>uuid;fundingPureId; fundingExternalId;funder_uuid;fundingClassification_uri;fn:string[@key='fundingProjectScheme'];fn:boolean[@key='financial'];fn:number[@key='awardedAmount'];fn:number[@key='institutionalPart'];visibility</xsl:text>
		...
		<xsl:for-each select="$json-xml/fn:map/fn:array[@key='items']/fn:map">
			...
			<xsl:for-each select="fn:array[@key='fundings']/fn:map">
				...
				<xsl:value-of select="fn:number[@key='awardedAmount']"/>
				<xsl:value-of select="$separator"/>
