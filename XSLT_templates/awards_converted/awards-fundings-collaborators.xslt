...
<xsl:text>uuid;fcFundingPureId; fcFunder_uuid;fcName_text_ca_ES;fcName_text_en_GB;fcName_text_es_ES;colType_term_text_ca_ES;colType_term_text_en_GB;colType_term_text_es_ES;fcInstitutionalPart</xsl:text>
		...
		<xsl:for-each select="$json-xml/fn:map/fn:array[@key='items']/fn:map">
			...
			<xsl:for-each select="fn:array[@key='fundings']/fn:map/fundingCollaborators/fundingCollaborator">
				...
				<!-- uuid del funder -->
				<xsl:value-of select="collaborator/fn:string[@key='uuid']"/>
				<xsl:value-of select="$separator"/>
				<xsl:value-of select="uab:clean_ca_en_es(collaborator/name)"/>
				<xsl:value-of select="$separator" />

				<!--tipus de funder  -->
				<xsl:value-of select="uab:clean_ca_en_es(collaborator/fn:map[@key='type']/fn:map[@key='term'])"/>
				<xsl:value-of select="$separator" />
				
				<!-- fn:number[@key='institutionalPart'] -->
				<xsl:value-of select="fn:number[@key='institutionalPart']"/>
				<xsl:value-of select="$separator"/>
