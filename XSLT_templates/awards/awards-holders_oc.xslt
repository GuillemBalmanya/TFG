...
<xsl:stylesheet version="3.0"
	...
	<xsl:template match="/">
		...
		<xsl:for-each select="$json-xml/fn:map/fn:array[@key='items']/fn:map">
			...
			<!-- JSON: awardHolders (camelCase) en lloc de awardholders/awardholder -->
			<xsl:for-each select="fn:array[@key='awardHolders']/fn:map">
				...
				<xsl:value-of select="$separator" />
				<!-- externalPerson/@uuid → externalPerson.uuid -->
				<xsl:value-of select="fn:map[@key='externalPerson']/fn:string[@key='uuid']"/>
				<xsl:value-of select="$separator" />
				<!-- externalPerson/type/@pureId → externalPerson.type.pureId -->
				...
				<xsl:value-of select="$separator" />
				<!-- externalPerson/@externalId → externalPerson.externalId -->
				...
				<xsl:value-of select="$separator" />
				<!-- personRole → role (canvi de nom al JSON) -->
				...
				<xsl:value-of select="fn:map[@key='period']/fn:string[@key='startDate']"/>
				<xsl:value-of select="$separator" />
				<xsl:value-of select="fn:map[@key='period']/fn:string[@key='endDate']"/>
				<xsl:value-of select="$separator" />
